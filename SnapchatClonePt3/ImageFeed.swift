...
func addPost(postImage: UIImage, thread: String, username: String) {
    ...
    let date = df.string(from: d as Date)
    let dict = [firImagePathNode : path, firThreadNode : thread, firUsernameNode : username, firDateNode : date] as [String : Any]
    print(dict)
        ...
        if snapshot.exists() {
            if let dict = snapshot.value as? [String : AnyObject] {
                ...
                for key in dict.keys {
                    ...
                    let id = key
                    let username = dict[key]?[firUsernameNode] as? String
                    let postImagePath = dict[key]?[firImagePathNode] as? String
                    let thread = dict[key]?[firThreadNode] as? String
                    let dateString = dict[key]?[firDateNode] as? String
                    if (!(username == nil || postImagePath == nil || thread == nil || dateString == nil)) {
                        let index = dateString?.index((dateString?.startIndex)!, offsetBy: 21)
                        let date = dateString?.substring(to: index!)
                    let post = Post(id: id, username: username!, postImagePath: postImagePath!, thread: thread!, dateString: date!, read: readPost!)
