...
class CurrentUser {
    ...
    */
    func getReadPostIDs(completion: @escaping ([String]) -> Void) {
        ...
        ref.child(firUsersNode).child(id).child(firReadPostsNode).observeSingleEvent(of: .value, with: {
            (snapshot) in
            if snapshot.exists() {
    ...
    */
    func addNewReadPost(postID: String) {
        ...
        ref.child(firUsersNode).child(id).child(firReadPostsNode).childByAutoId().setValue(postID)
    }
